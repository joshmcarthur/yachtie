defmodule YachtieWeb.ManualControlLive do
  ...
  def handle_info({:temperature_celsius, temp}, socket), do: {:noreply, assign(socket, temperature: temp)}
  def handle_info({:humidity_rh, humidity}, socket), do: {:noreply, assign(socket, humidity: humidity)}
  ...
  def handle_info({:gps_satellites, sats}, socket), do: {:noreply, assign(socket, satellites: sats)}
  def handle_info({:gps_speed, speed_knots}, socket), do: {:noreply, assign(socket, speed_knots: speed_knots)}
  def handle_info({:gps_course, course_deg}, socket), do: {:noreply, assign(socket, course_degrees: course_deg)}
  def handle_info({:gps_location, latlng}, socket), do: {:noreply, assign(socket, location: String.split(latlng, ","))}
